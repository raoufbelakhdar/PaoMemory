...
import { ChevronLeft, Copy, Check, Upload, AlertCircle, CheckCircle2, ArrowRight, Sparkles } from 'lucide-react';
...
export function ImportFlow({ onComplete, onBack }: ImportFlowProps) {
  ...
  const convertPrompt = `I have a PAO (Person-Action-Object) memory system list. Please convert it to JSON format with this exact structure:
...
Requirements:
- Numbers go from 00-99
- Each number has exactly 3 entries: person (capitalized name), action (lowercase verb), object (lowercase noun)
- Leave imageUrl as empty string
- Return only the valid JSON array

My PAO list:
[Paste your PAO list here]`;

  const handleCopyPrompt = async () => {
    try {
      await navigator.clipboard.writeText(convertPrompt);
  ...
  const steps = [
    // Step 0: Welcome
    {
      title: 'Welcome',
      render: () => (
        <div className="min-h-screen bg-gradient-to-br from-chart-2/5 via-background to-emerald-500/5 flex flex-col relative overflow-hidden">
          {/* Animated background */}
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <div className="absolute top-20 right-10 w-80 h-80 bg-chart-2/10 rounded-full blur-3xl animate-pulse" />
            <div className="absolute bottom-20 left-10 w-64 h-64 bg-emerald-500/10 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1.5s' }} />
          </div>
              ...
              {[0, 1, 2, 3].map((i) => (
                <div
                  ...
                  className={`h-1 rounded-full transition-all duration-500 ${
                    i === 0 ? 'w-8 bg-chart-2 shadow-sm shadow-chart-2/50' : 'w-1 bg-muted/50'
                  }`}
                />
              ))}
            </div>
          ...
          <div className="relative flex-1 flex flex-col px-5 pb-6 justify-center">
            <div className="max-w-md mx-auto w-full text-center space-y-8">
              ...
              <div className="relative w-28 h-28 mx-auto">
                <div className="absolute inset-0 bg-gradient-to-br from-chart-2 to-emerald-500 rounded-[2rem] shadow-2xl shadow-chart-2/30 flex items-center justify-center">
                  <Upload className="w-14 h-14 text-white" strokeWidth={2} />
                </div>
                <div className="absolute -inset-1 bg-gradient-to-br from-chart-2/20 to-emerald-500/20 rounded-[2rem] blur-xl -z-10" />
              </div>

              {/* Title */}
              <div className="space-y-3">
                <h1 className="text-[2.5rem] leading-tight tracking-tight">
                  Let's import your PAO
                </h1>
                <p className="text-muted-foreground text-lg leading-relaxed">
                  We'll help you convert your existing list into the right format
                </p>
              </div>
              ...
              <div className="glass-strong backdrop-blur-2xl rounded-3xl p-6 text-left space-y-4 shadow-xl border border-white/10">
                {[
                  ...
                  'Automatic conversion with AI',
                  'Preview before importing',
                ].map((feature, i) => (
                  <div key={i} className="flex items-center gap-4">
                    <div className="w-7 h-7 rounded-xl bg-gradient-to-br from-chart-2/20 to-emerald-500/20 flex items-center justify-center flex-shrink-0">
                      <CheckCircle2 className="w-4 h-4 text-chart-2" strokeWidth={2.5} />
                    </div>
                    <span>{feature}</span>
                  </div>
                ))}
          ...
          <div className="relative px-5 pb-6">
            <Button
              ...
              className="w-full bg-gradient-to-r from-chart-2 to-emerald-500 hover:opacity-90 text-white rounded-2xl h-14 text-base shadow-2xl shadow-chart-2/20 border-0"
            >
              Continue
    ...
    // Step 1: Explain
    {
      ...
      render: () => (
        <div className="min-h-screen bg-gradient-to-br from-chart-2/5 via-background to-emerald-500/5 flex flex-col relative overflow-hidden">
          {/* Animated background */}
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <div className="absolute top-32 right-16 w-72 h-72 bg-chart-2/8 rounded-full blur-3xl animate-pulse" />
          </div>
            ...
            <div className="flex gap-1.5">
              {[0, 1, 2, 3].map((i) => (
                <div
                  key={i}
                  className={`h-1 rounded-full transition-all duration-500 ${
                    i === 1 ? 'w-8 bg-chart-2 shadow-sm shadow-chart-2/50' : i < 1 ? 'w-1 bg-chart-2/50' : 'w-1 bg-muted/50'
                  }`}
                />
              ))}
            </div>
          ...
          <div className="relative flex-1 flex flex-col px-5 pb-6">
            <div className="mb-6">
              ...
              <p className="text-muted-foreground">
                Three simple steps to get your PAO ready
              </p>
            </div>
            ...
            <div className="flex-1 space-y-3">
              {[
                {
                  number: '1',
                  title: 'Copy our prompt',
                  description: 'We provide a ChatGPT prompt for you',
                  color: 'from-primary to-chart-1',
                },
                {
                  number: '2',
                  title: 'Paste your PAO list',
                  description: 'Add your list to the prompt and send to ChatGPT',
                  color: 'from-chart-2 to-emerald-500',
                },
                {
                  number: '3',
                  title: 'Import the result',
                  description: 'Copy the JSON output and paste it here',
            ...
            <Button
              ...
              className="w-full bg-gradient-to-r from-chart-2 to-emerald-500 hover:opacity-90 text-white rounded-2xl h-14 text-base shadow-2xl shadow-chart-2/20 border-0 mt-6"
            >
              I'm ready
              <ArrowRight className="w-5 h-5 ml-2" />
            </Button>
        ...
        <div className="min-h-screen bg-gradient-to-br from-chart-2/5 via-background to-emerald-500/5 flex flex-col relative overflow-hidden">
            ...
            <div className="flex gap-1.5">
              {[0, 1, 2, 3].map((i) => (
                <div
                  key={i}
                  className={`h-1 rounded-full transition-all duration-500 ${
                    i === 2 ? 'w-8 bg-chart-2 shadow-sm shadow-chart-2/50' : i < 2 ? 'w-1 bg-chart-2/50' : 'w-1 bg-muted/50'
                  }`}
                />
              ))}
            </div>
          ...
          <div className="flex-1 flex flex-col px-5 pb-6">
            <div className="mb-5">
              ...
              <p className="text-muted-foreground">
                Use it with ChatGPT to convert your PAO list
              </p>
            </div>
            ...
            <div className="flex-1 glass-strong backdrop-blur-2xl rounded-3xl overflow-hidden shadow-xl flex flex-col border border-white/10 min-h-0">
              <div className="flex items-center justify-between px-4 py-3.5 border-b border-border/50 flex-shrink-0">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-sm">Conversion Prompt</span>
                </div>
                <Button
                  size="sm"
                  variant={copiedPrompt ? "default" : "ghost"}
                  className={`h-9 px-3 rounded-lg transition-all ${copiedPrompt ? 'bg-chart-2 hover:bg-chart-2/90 shadow-lg' : ''}`}
            ...
            <Button
              ...
              className="w-full bg-gradient-to-r from-chart-2 to-emerald-500 hover:opacity-90 text-white rounded-2xl h-14 text-base shadow-2xl shadow-chart-2/20 border-0 mt-5"
            >
              I have my JSON ready
              <ArrowRight className="w-5 h-5 ml-2" />
            </Button>
          </div>
        </div>
            ...
            <div className="flex gap-1.5">
              {[0, 1, 2, 3].map((i) => (
                <div
                  key={i}
                  className={`h-1 rounded-full transition-all duration-500 ${
                    i === 3 ? 'w-8 bg-chart-2 shadow-sm shadow-chart-2/50' : i < 3 ? 'w-1 bg-chart-2/50' : 'w-1 bg-muted/50'
                  }`}
                />
              ))}
            </div>
            ...
            {importedData.length === 0 ? (
              ...
              <>
                <div className="mb-5 flex-shrink-0">
                  ...
                  <p className="text-muted-foreground">
                    Import the formatted data from ChatGPT
                  </p>
                </div>
                ...
                <div className="flex-1 flex flex-col gap-4 min-h-0">
                  ...
                  <div className="flex-1 min-h-0">
                    <textarea
                      ...
                      className="w-full h-full p-4 rounded-2xl glass-strong backdrop-blur-2xl border border-white/10 focus:border-chart-2 focus:outline-none focus:ring-2 focus:ring-chart-2/20 resize-none font-mono text-sm shadow-xl transition-all"
                    />
                  </div>
                  ...
                  <label htmlFor="file-upload" className="block flex-shrink-0">
                    <div className="glass-strong backdrop-blur-2xl rounded-2xl p-5 border-2 border-dashed border-border/50 hover:border-chart-2 cursor-pointer transition-all text-center shadow-lg hover:shadow-xl hover:scale-[1.01] active:scale-[0.99]">
                      <Upload className="w-9 h-9 mx-auto mb-2.5 text-chart-2" />
                  ...
                  {error && importedData.length === 0 && (
                    <div className="flex items-start gap-3 p-4 rounded-2xl glass-strong backdrop-blur-2xl border border-destructive/50 shadow-lg flex-shrink-0">
                      ...
                      <div className="flex-1">
                        ...
                        <button
                          onClick={() => setCurrentStep(2)}
                          className="text-xs text-chart-2 underline underline-offset-2"
                        >
                ...
                <Button
                  ...
                  className="w-full bg-gradient-to-r from-chart-2 to-emerald-500 hover:opacity-90 text-white rounded-2xl h-14 text-base shadow-2xl shadow-chart-2/20 border-0 disabled:opacity-50 disabled:cursor-not-allowed mt-5 flex-shrink-0"
                >
                  Import & Preview
                  <ArrowRight className="w-5 h-5 ml-2" />
                </Button>
              ...
              <>
                <div className="mb-5 flex-shrink-0">
                  <div className="flex items-center gap-2.5 mb-3">
                    <div className="w-8 h-8 rounded-xl bg-chart-2/10 flex items-center justify-center">
                      <CheckCircle2 className="w-5 h-5 text-chart-2" strokeWidth={2.5} />
                    </div>
                ...
                <Button
                  ...
                  className="w-full bg-gradient-to-r from-chart-2 to-emerald-500 hover:opacity-90 text-white rounded-2xl h-14 text-base shadow-2xl shadow-chart-2/20 border-0 flex-shrink-0"
                >
                  Looks good! Continue
                  <ArrowRight className="w-5 h-5 ml-2" />
                </Button>
